...
* **Repositorio:** [https://github.com/AlejandroFeriaGonzalez/RNAB](https://github.com/AlejandroFeriaGonzalez/RNAB)

* **Pagina web:** [http://yo4scgkgckg4wgwggo4kgkc4.35.206.85.70.sslip.io](http://yo4scgkgckg4wgwggo4kgkc4.35.206.85.70.sslip.io/demand-prediction)

* **Notebooks:** [https://github.com/AlejandroFeriaGonzalez/RNAB/tree/main/notebooks](https://github.com/AlejandroFeriaGonzalez/RNAB/tree/main/notebooks)

* **Video aportes individuales:**[https://drive.google.com/file/d/1tnkYNpNTVe4NPTDkbZOSaxwdcPg3Hk9C/view?usp=sharing](https://drive.google.com/file/d/1tnkYNpNTVe4NPTDkbZOSaxwdcPg3Hk9C/view?usp=sharing)

* **Carpeta en drive con adjuntos:**: [https://drive.google.com/drive/folders/1lu5ueq6-29qSGLfCo9F5SWdy1mWegYpg?usp=drive_link](https://drive.google.com/drive/folders/1lu5ueq6-29qSGLfCo9F5SWdy1mWegYpg?usp=drive_link)
...
El proyecto se centró en la creación de tres módulos: un modelo de series de tiempo para predecir la demanda de transporte con 30 días de antelación, un clasificador de imágenes para identificar conductas distractivas como el uso del teléfono móvil, y un sistema de recomendación que sugiere destinos personalizados a los usuarios basándose en su historial y preferencias. Todas estas soluciones se integraron en una herramienta web intuitiva creada con FastAPI. Para la realizacion de los modelos 2 y 3 se uso TensorFlow y Keras, mientras que el modelo de series de tiempo se implementó con PyTorch.
...
* Librerías de Aprendizaje Profundo: TensorFlow, Keras, Scikit-learn, PyTorch
 ...
 **RMSE (Root Mean Square Error):** 0.9165

  El RMSE mide la diferencia promedio entre los valores predichos y los valores reales, penalizando en mayor medida los errores más grandes. Un valor 0.9165 indica que en promedio, las predicciones se desvían aproximadamente 0.9165 unidades de los valores reales.

  El MAE mide la diferencia absoluta promedio entre las predicciones y los valores reales.

**MAE (Mean Absolute Error):** 0.3289

  El MAE mide la diferencia absoluta promedio entre las predicciones y los valores reales. Un valor 0.3289 indica que en promedio, las predicciones se desvían aproximadamente 0.3289 unidades de los valores reales.

Si bien se presentan los valores de un modelo moderado, se debe tener en cuenta que se está analizando un modelo de clasificación para detectar distracciones. Por ende, el RMSE y el MAE no son las métricas más adecuadas para evaluar este problema. Es preferible utilizar precision, recall, f1-score y support, con los cuales se obtiene un rendimiento BUENO, dado un accuracy del 84%.
...
* **RMSE (Root Mean Square Error):** 0.1889
Las predicciones de ratings se desvían en promedio 0.1889 unidades de los valores reales.
Un RMSE tan bajo indica predicciones muy precisas
* **MAE (Mean Absolute Error):** 0.1265
En promedio, las predicciones se desvían 0.1265 unidades de los ratings reales. Un MAE tan bajo muestra alta precisión

#### Comparación de Métricas:

* **RMSE (0.1889) > MAE (0.1265):** Esto es normal y esperado
* **Diferencia pequeña:** Indica que hay pocos errores grandes (outliers)
* **Consistencia:** Ambas métricas muestran excelente rendimiento

#### Métricas de Clasificación:

* **Precisión promedio:** 0.06 (6%)
Solo el 6% de las recomendaciones contienen el destino relevante. Necesita mejora.
* **Recall promedio:** 0.17 (17%)
El sistema logra recomendar el destino relevante en el 17% de los casos. Necesita mejora.

#### Análisis General del Sistema:

* **Fortalezas:**
  * Predicción de ratings: Excelente (RMSE: 0.1889, MAE: 0.1265)
  * Sistema híbrido: Combina filtrado colaborativo y por contenido
  * Validación cruzada: Implementada correctamente
  * Feedback del usuario: Sistema de calificación integrado

* **Limitaciones:**
  * Baja precisión de recomendaciones: 6% precision, 17% recall
  * Pocos destinos: Dataset limitado en diversidad
  * Datos simulados: Pueden no reflejar patrones reales
  * Falta de personalización: Recomendaciones muy genéricas
...
#### Recomendaciónes y cosas a mejorar:

El modelo es adecuado para predecir calificaciones pero necesita mejoras significativas en la generación de recomendaciones.

* Expandir el dataset con más destinos

* Implementar algoritmos más sofisticados (SVD, NMF)

* Mejorar la personalización basada en preferencias específicas

* Con pocos destinos, la diversidad es baja y es difícil acertar exactamente el destino relevante.

* El sistema no está "adivinando" lo que el usuario quiere, sino que intenta predecir el pasado, lo cual no siempre es realista.
...
### Módulo de Predicción de Demanda
  ...
  * Descripción: Permite a los usuarios subir un archivo CSV con datos históricos para predecir la demanda futura de transporte y visualizar la predicción de demanda para los próximos 30 días. Tambien pueden escribir la cantidad de días a predecir y la logitud de la secuencia, es decir cuantos dias de datos se usan para predecir los dìas siguientes.
  * Interfaz:
    ![Captura de Pantalla de la Interfaz de Predicción de Demanda](form1.png)
    Descripción: Captura de pantalla de la sección de predicción de demanda. Se puede ver el input para subir el archivo CSV, un campo para ingresar la cantidad de días a predecir y la longitud de la secuencia, y un botón para enviar los datos. Una vez procesado, se muestra una tabla con las predicciones.

### Módulo de Clasificación de Conducción Distractiva

  * Descripción: Permite a los usuarios subir una imagen de un conductor. El sistema procesa la imagen y devuelve la clasificación del comportamiento detectado, junto con la probabilidad de cada clase.

  * Interfaz:
    ![Captura de Pantalla de la Interfaz de Clasificación de Imágenes](form2.png)
...
### Módulo de Recomendación de Destinos de Viaje
  ...
  * Interfaz:
    ![Captura de Pantalla de la Interfaz de Recomendación de Destinos](form3.png)
    Descripción: Captura de pantalla de la sección de recomendación, mostrando un formulario con las preferencias del usuario para asi obtener recomendaciones personalizadas. Incluye campos para seleccionar tipo de destino, época del año y estado, así como un botón para enviar la solicitud. Los resultados se muestran en una lista.
...
Afridi, A. S. (2024). *Multi-Class Driver Behavior Image Dataset* [Conjunto de datos]. Kaggle. https://www.kaggle.com/datasets/arafatsahinafridi/multi-class-driver-behavior-image-dataset

Chollet, F., et al. (2015). *Keras*. https://keras.io

Consorcio Regional de Transportes de Madrid (CRTM). (2025). *Viajeros transportados por modo de transporte* [Conjunto de datos]. Portal de Datos Abiertos del CRTM. https://datos.crtm.es/documents/a7210254c4514a19a51b1617cfd61f75/about
...
Mehra, A. (2024). *Travel Recommendation Dataset* [Conjunto de datos]. Kaggle. https://www.kaggle.com/datasets/amanmehra23/travel-recommendation-dataset
...
* Video con los aportes de cada integrantes: [https://drive.google.com/file/d/1tnkYNpNTVe4NPTDkbZOSaxwdcPg3Hk9C/view?usp=sharing](https://drive.google.com/file/d/1tnkYNpNTVe4NPTDkbZOSaxwdcPg3Hk9C/view?usp=sharing)

* Codigo fuente del proyecto: [Repositorio en GitHub](https://github.com/AlejandroFeriaGonzalez/RNAB)

* Dataset de Predicción de Demanda: [https://datos.crtm.es/documents/a7210254c4514a19a51b1617cfd61f75/about](https://datos.crtm.es/documents/a7210254c4514a19a51b1617cfd61f75/about)
